...
- Scope wasn't written down until message six. The first design assumed physical cash counting and had to be rebuilt. Writing a one-paragraph scope statement first would have caught it in an hour instead of a day. - Column indices were positional and broke the moment email collection shifted every column right by one. Header-based lookup should have been the first design, not the fix. - The roster lives in two places (form checkboxes and Config) and must be spelled identically. That's the most likely thing to break after handoff.
